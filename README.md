...
- **案例档案** —— 排盘后自动保存完整 JSON 快照；支持搜索、解读版本（可删除不满意的历史版本）、反馈、跨设备批量迁移；占问文本与背景问念均可在档案中补充修改。
- **灵活导出** —— 单档 Markdown / JSON / PNG 长图导出：可选择只分享卦本身（不带解读与反馈）、或只带最新一版解读；历史版本默认口径首次导出时选择，之后可在设置中修改；长图信息卡高度随占问/问念长度自适应、完整不截断；JSON 与迁移包始终保持完整数据。
- **放大镜与字体** —— 详情页一键放大查看（1.5×/2×/2.5× 全屏只读拖动，以屏幕中心为锚点放大、切换倍数不打断注视位置、双击复位，不影响排盘排版与按键）；产品内字体与导出字体可在设置中分别选择（系统默认 / 道谕宋），切换即时生效。
- **输入防丢失** —— 解读内容实时存草稿（按档案隔离），误退出或应用被杀后重进自动恢复；未保存退出前弹确认；反馈与占问/问念编辑同样有关闭前确认。
