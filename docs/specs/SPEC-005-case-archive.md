...
- 档案页支持批量导出版本化迁移包；迁移包包含每一条完整卦面快照、现存全部解读版本和全部反馈记录。
- 2026-09-04 起：档案详情可直接编辑「占问」文本本身（起卦时输入有误可纠正），保存后写入 `questionUpdatedAt` 溯源时间戳，起卦记录、历法快照与卦面快照不变；「编辑背景问念」继续作为独立补充通道保留。
- 2026-09-04 起：解读历史版本支持删除（写得不好或有误的版本不必保留），删除前二次确认；`latestAnalysisRevision` 不因删除回退，版本号单调递增并允许空洞，乐观锁与后续追加不受影响。
...
- `FR-CAS-005` 编辑问题或备注不得修改原始起卦和卦面快照。（2026-09-04 细化：允许修改占问文本本身——仅更新 `question` 与 `questionUpdatedAt`，起卦原始记录、历法快照、卦面快照及四柱上下文一律不动。）
- `FR-CAS-017` 删除单个解读版本必须二次确认；删除后其余版本、反馈与卦面不受影响，`latestAnalysisRevision` 保持不回退，后续新版本 revision 取 `latestAnalysisRevision + 1`（不复用删除留下的空洞）。
- `FR-CAS-018`（2026-09-05）解读编辑防丢失双层保护：① 输入文本实时落盘为按档案隔离的草稿（应用文档目录根部 `draft-analysis-<safe>.txt`，`<safe>` 为 caseId 过滤安全字符后的文件名，尽力而为、失败静默），保存为正式版本、清空或删除档案时删除草稿，重新进入详情页自动恢复并提示；② 存在未保存变更时拦截返回（PopScope），弹出「保存并退出 / 直接退出 / 继续编辑」三选。反馈与文本弹层编辑器在关闭前对未保存修改做二次确认，且编辑类弹层禁用下滑关闭（下滑走 `Navigator.pop` 不经 `maybePop`，会绕过确认）。
...
| 日期 | 版本 | 变更 | 证据 |
|---|---|---|---|
| 2026-09-05 | 0.8 | 解读草稿双层保护（实时落盘 + 返回拦截三选）；反馈/弹层编辑器关闭前确认未保存修改 | DEC-052、draft_store 单测、widget 拦截测试 |
| 2026-09-04 | 0.7 | 占问文本可编辑（不触碰起卦与卦面快照，新增 `questionUpdatedAt` 溯源）；解读历史版本可删除（二次确认、revision 不回退） | DEC-051、archive_offline_test 22 项、widget 全量 |
