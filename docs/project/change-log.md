...
- **v1.18.2 发布口径修正**（v1.18.2 发布后用户反馈「降级安装不了 + 包体变大许多」）：首发误按 README 记载的默认 fat 构建上传（60MB、三 ABI、versionCode 33），而已发布的 v1.18.0/v1.18.1 均为 arm64 split 包（24MB、versionCode = 2000 + build number，v1.18.1 即 2032）——33 < 2032 触发 Android 降级拒绝，fat 包体积约为 split 包 2.6 倍。用项目既定口径 `flutter build apk --release --target-platform android-arm64 --split-per-abi` 重建（24MB、versionCode 2033 > 2032 可覆盖安装，aapt2 核验）并通过断网门禁，同名覆盖替换 Release 资产。**README 构建命令同步修正**为 arm64 split 口径并注明 versionCode 规则与踩坑原因（README 原命令与 change-log 第十二批补充已确立的 split-per-abi 口径不一致是本次误用的直接根源）。
